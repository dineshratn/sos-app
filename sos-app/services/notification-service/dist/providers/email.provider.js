...
class EmailProvider {
    ...
    initialize() {
        try {
            if (!config_1.config.sendgrid.apiKey) {
                logger_1.logger.warn('SendGrid API key not configured, email provider disabled');
                return;
            }
            mail_1.default.setApiKey(config_1.config.sendgrid.apiKey);
            this.initialized = true;
            logger_1.logger.info('SendGrid email provider initialized successfully');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize SendGrid email provider', {
                error: error.message,
            });
        }
    }
        ...
        try {
            const msg = {
                ...
                from: {
                    ...
                    name: config_1.config.sendgrid.fromName,
                },
                replyTo: config_1.config.sendgrid.fromEmail,
     ...
     */
    async sendBulkEmail(emails, job) {
        if (!this.initialized) {
        ...
        try {
            const responses = await mail_1.default.send(messages);
            ...
            logger_1.logger.error('Failed to send bulk email', {
                emergencyId: job.emergencyId,
                error: error.message,
            });
            return emails.map(() => ({
